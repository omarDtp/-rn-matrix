...
class DehydrationManager {
  ...
  async getDehydrationKeyFromCache() {
    return this.crypto._cryptoStore.doTxn('readonly', [_indexeddbCryptoStore.IndexedDBCryptoStore.STORE_ACCOUNT], txn => {
  ...
  async setDehydrationKey(key, keyInfo = {}, deviceDisplayName = undefined) {
    if (!key) {
      ...
      this.crypto._cryptoStore.doTxn('readwrite', [_indexeddbCryptoStore.IndexedDBCryptoStore.STORE_ACCOUNT], txn => {
    ...
    if (!matches) {
      this.key = key;
      this.keyInfo = keyInfo;
      this.deviceDisplayName = deviceDisplayName; // start dehydration in the background
