...
Crypto.prototype.bootstrapSecretStorage = async function ({
  ...
  const createSSSS = async (opts, privateKey) => {
    ...
    const keyId = await secretStorage.addKey(_SecretStorage.SECRET_STORAGE_ALGORITHM_V1_AES, opts);
    ...
    if (!keyInfo.mac) {
      ...
      if (key) {
        ...
        const {
          iv,
          mac
        } = await _SecretStorage.SecretStorage._calculateKeyCheck(keyData);
        keyInfo.iv = iv;
        keyInfo.mac = mac;
  ...
  if (!storageExists && !keyBackupInfo) {
    ...
    if (this._crossSigningInfo.getId() && this._crossSigningInfo.isStoredInKeyCache("master")) {
